...
VALID_DONATION_STATUSES = {"AVAILABLE", "MATCHED", "PICKUP_PENDING", "PICKUP_ASSIGNED", "PICKED_UP", "COLLECTED", "DISTRIBUTED", "DELIVERED", "CANCELLED", "EXPIRED"}
...
def create_donation(
    ...
    database.create_notification_record(
        ...
        "donor",
        clean_donor,
        f"Donation {donation_id} created successfully.",
        "console"
    )
...
def update_donation_status(donation_id: Optional[str] = None, status: str = "") -> str:
    ...
    if norm_status not in VALID_DONATION_STATUSES:
        return json.dumps({
            "status": "error",
            "message": f"Invalid status '{status}'. Allowed statuses: {sorted(list(VALID_DONATION_STATUSES))}"
        })
...
def accept_donation(donation_id: Optional[str] = None, organization_id: Optional[str] = None) -> str:
    ...
    if donation["status"] in ["COLLECTED", "DELIVERED", "CANCELLED", "EXPIRED"]:
        return json.dumps({
            "status": "error", 
            "message": f"Cannot accept donation '{clean_don_id}' because its status is already {donation['status']}."
        })
    ...
    if success:
        database.create_notification_record(
            ...
            "organization",
            clean_org_id,
            f"You have successfully accepted donation {clean_don_id}.",
            "console"
        )
...
def find_available_volunteers(location: Optional[str] = None) -> str:
    ...
    return json.dumps({
        ...
        "count": len(vols),
        "volunteers": vols,
        "message": f"Found {len(vols)} available volunteer(s) in '{target_loc}'."
    }, indent=2)
...
def create_pickup_task(
    ...
    clean_org_id = str(organization_id).strip() if organization_id and str(organization_id).strip() else _get_context_val("current_organization_id")
    clean_pickup_loc = str(pickup_location).strip() if pickup_location and str(pickup_location).strip() else _get_context_val("current_location")
    clean_time = str(scheduled_time).strip() if scheduled_time and str(scheduled_time).strip() else _get_context_val("current_pickup_deadline", "Immediate")
    ...
    record = database.create_pickup_task_record(
        ...
        donation_id=clean_don_id,
        org_id=clean_org_id,
        pickup_loc=clean_pickup_loc,
        delivery_loc=clean_delivery_loc,
        time=clean_time
    )
...
def assign_volunteer(task_id: Optional[str] = None, volunteer_id: Optional[str] = None) -> str:
    ...
    if success:
        database.create_notification_record(
            ...
            "volunteer",
            clean_vol_id,
            f"You have been assigned to pickup task {clean_task_id}.",
            "console"
        )
...
def update_pickup_status(task_id: Optional[str] = None, status: str = "") -> str:
    ...
    if norm_status not in VALID_PICKUP_STATUSES:
        return json.dumps({
            "status": "error",
            "message": f"Invalid pickup status '{status}'. Allowed: {sorted(list(VALID_PICKUP_STATUSES))}"
        })
        ...
        try:
            ...
            if loop.is_running():
                ...
                res = asyncio.run_coroutine_threadsafe(
                    provider.compute_route(target_origin, target_dest, target_mode), loop
                ).result(timeout=3.0)
            else:
                res = loop.run_until_complete(
                    routing.calculate_route(target_origin, target_dest, target_mode)
                )
        except Exception:
...
def create_reimbursement(
    ...
    target_vol_id = str(volunteer_id).strip() if volunteer_id and str(volunteer_id).strip() else task.get("volunteer_id") or _get_context_val("current_volunteer_id")
...
def update_reimbursement_status(
    ...
    if norm_status not in {"PENDING", "APPROVED", "PAID", "CANCELLED"}:
        return json.dumps({
            "status": "error",
            "message": f"Invalid status '{status}'. Allowed: ['PENDING', 'APPROVED', 'PAID', 'CANCELLED']"
        })
...
def register_donor(
    ...
    if existing:
        ...
        record = database.create_donor_record(
            donor_id=donor_id,
            name=clean_name,
            phone=clean_phone,
            location=clean_loc,
...
def get_my_donations(phone: Optional[str] = None, donor_id: Optional[str] = None) -> str:
    ...
    return json.dumps({
        "status": "not_found",
        "message": "No donations found for your account."
    }, indent=2)
...
def get_my_pickups(
    ...
    if not tasks:
        return json.dumps({
            "status": "not_found",
            "message": "No pickup tasks currently found."
        }, indent=2)
...
def cancel_donation(donation_id: Optional[str] = None, reason: Optional[str] = None) -> str:
    ...
    if existing["status"] in ["DELIVERED", "CANCELLED"]:
        return json.dumps({
            "status": "error",
            "message": f"Donation '{target_id}' is already {existing['status']} and cannot be cancelled."
        })
...
def update_volunteer_availability(
    ...
    success = database.update_volunteer_availability(
        ...
        current_coordinates=coords_dict
    )
...
def get_available_volunteers(
    ...
    volunteers = database.get_available_volunteers(
        service_area=target_area or None,
    ...
    if norm_type in ["DONOR_PICKUP", "PICKUP"]:
        if target_task_id:
            database.update_pickup_task_logistics(
                ...
                pickup_coordinates=coords_dict
            )
        if target_don_id:
            database.update_donation_details_record(
                donation_id=target_don_id,
        ...
        if target_vol_id:
            database.update_volunteer_availability(
                volunteer_id=target_vol_id,
                status="AVAILABLE",
                current_location=address or f"{lat:.4f}, {lng:.4f}",
                current_coordinates=coords_dict
            )
        return json.dumps({
            "status": "success",
        ...
        if target_task_id:
            database.update_pickup_task_logistics(
                task_id=target_task_id,
                destination_coordinates=coords_dict
            )
        return json.dumps({
            "status": "success",
...
def confirm_pickup(
    ...
    return json.dumps({
        ...
        "volunteer_instructions": f"✅ Pickup recorded. Next step: Deliver to {org_name} at {dest_loc}." + (f" 📍 {dest_map_link}" if dest_map_link else ""),
...
def confirm_delivery(
    ...
    return json.dumps({
        ...
        "reimbursement": {
            "distance_km": dist,
            "transport_mode": mode,
            "estimated_support": reimb_amount,
            "currency": "LKR"
        },
