...
async def send_whatsapp_image(
    to_number: str, image_url: str, caption: Optional[str] = None, reply_to_message_id: Optional[str] = None
) -> Dict[str, Any]:
...
async def dispatch_lifecycle_cross_notifications(prompt_text: str, reply_text: str, from_number: str) -> None:
    ...
    is_accepted_intent = (is_accept_text and (is_accept_state or has_vol_accept_reply or is_vol_user) and not is_org_user and not is_donor_user) or (
        has_vol_accept_reply and not is_org_user and not is_donor_user
    )
    ...
    if is_delivered_intent:
        ...
        if target_task:
            ...
            if vol:
                ...
                database.update_volunteer_availability(vol["id"], "BUSY")
            else:
            ...
            ext = resilient_executor._get_task_extended_metrics(target_task, vol)
            total_dist = ext.get("total_dist", 5.0)
            est_cost = ext.get("est_cost", 350)

            # 1. Send Waiting Message to Volunteer
            vol_user = database.get_user_by_phone(from_number)
            v_lang = vol_user.get("preferred_language", "en") if vol_user else "en"
            v_wait_msg = translation_service.get_localized_message(
            ...
            try:
                await send_whatsapp_message(to_number=from_number, text=v_wait_msg)
            except Exception as e:
                logger.warning(f"Failed to send volunteer waiting notification: {e}")
            try:
                database.record_message(phone=from_number, sender="agent", text=v_wait_msg)
            except Exception:
                pass
            ...
            if org_phone:
                ...
                try:
                    ...
                    logger.warning(f"Failed to send WhatsApp organization courier confirmation request: {e}")
                try:
                    database.record_message(phone=org_phone, sender="agent", text=o_msg)
                except Exception:
                    pass
    ...
    # 4. Donor Confirms Donation -> Send Offer Requests to District Recipient Organizations (Ordered by Distance)
        ...
        w in reply_text.lower()
        for w in ["notified nearby recipient organizations", "donation confirmed!", "notified registered recipient organizations"]
    ):
        ...
        if top_don:
            ...
            don_dist = routing.resolve_district(pickup_loc) or (pickup_loc if pickup_loc != "Pickup Location" else "Colombo")
            donor_coords = None
            ...
            for org in dist_orgs:
                ...
                if o_phone and o_phone != from_number:
                    ...
                    if donor_coords and o_coords:
                        d_km = round(
                            max(0.5, routing.calculate_haversine_distance(donor_coords[0], donor_coords[1], o_coords[0], o_coords[1]) * 1.25), 1
                        )
                    else:
                        calc_rd = routing.calculate_road_distance(pickup_loc, o_loc)
                        d_km = round(calc_rd, 1) if calc_rd > 0 else 5.0
                    ...
                    try:
                        await send_whatsapp_message(to_number=o_phone, text=o_msg)
                    except Exception as e:
                        logger.warning(f"Failed to send donation offer to org {o_phone}: {e}")

    # 5. Organization Accepts Donation Offer OR Donor Accepts Organization
    elif any(
        w in reply_text.lower()
        for w in ["food donation accepted!", "confirmed acceptance of", "connected with", "we have sent your donation details to"]
    ):
        all_tasks = database.get_all_pickup_tasks()
        ...
        if active_tasks:
            ...
            donor_loc = (
                (don.get("pickup_location") if don else None)
                or (don.get("location") if don else None)
                or top_task.get("pickup_location")
                or "Pickup Location"
            )
            food_info = _format_food_info(don)
            ...
            task_dist = (
                routing.resolve_district(donor_loc)
                or routing.resolve_district(org.get("location") if org else "")
                or (donor_loc if donor_loc != "Pickup Location" else "Colombo")
            )

            # Case A: Donor accepted the match (from_number != org_phone) -> Notify Org with full donor details
            ...
            # Case B: Org accepted the match (from_number == org_phone) -> Notify Donor
            ...
            # Search best available volunteer strictly in this district to offer the pickup task
    ...
    # 6. Organization Match Proposed to Donor or Matched
        ...
        w in reply_text.lower()
        for w in ["matched an organization", "connected to", "we found a recipient organization", "found an available food match"]
    ):
        curr_state = database.get_user_conversation_state(from_number) or {}
...
async def dispatch_qr_pickup_success_notifications(task_id: str, volunteer_id: Optional[str] = None) -> None:
    ...
    # 2. Notify Organization with Delivery QR Image (No verification text link in chat)
    org_phone = org.get("phone") if org else None
    if org_phone:
        org_user = database.get_user_by_phone(org_phone)
        o_lang = org_user.get("preferred_language", "en") if org_user else "en"
        ...
        try:
            database.record_message(phone=org_phone, sender="agent", text=f"{o_msg}\n\n📷 [Delivery QR Code Image]({dl_qr_img})")
        except Exception:
            pass
...
async def dispatch_qr_delivery_success_notifications(task_id: str, volunteer_id: Optional[str] = None) -> None:
    ...
    if org_phone:
        ...
        o_lang = org_user.get("preferred_language", "en") if org_user else "en"
        o_msg = translation_service.get_localized_message(
            "qr_delivery_verified_org", lang=o_lang, food_info=food_info, volunteer_name=vol_name, task_id=task_id, timestamp=now_str
        )
        try:
            await send_whatsapp_message(to_number=org_phone, text=o_msg)
        except Exception as e:
    ...
    # 3. Notify Volunteer with completion & availability prompt
    if vol_phone:
        vol_user = database.get_user_by_phone(vol_phone)
        v_lang = vol_user.get("preferred_language", "en") if vol_user else "en"
        v_msg = translation_service.get_localized_message(
    ...
    if msg_type == "text":
        ...
        if not is_new_user and is_greeting and not has_active_food_draft and not is_voice_message:
            if vol_rec or (user and user.get("user_role") == "volunteer"):
                ...
                reply_text = translation_service.get_localized_message("returning_donor_welcome", lang=preferred_language, name=name)
            else:
    ...
    # 3. Location message (Meta Cloud API location payload)
        ...
        if is_vol:
            ...
            if not vol_rec:
                ...
                v_veh_reg = conv_state.get("vol_vehicle") or "Motorbike"
                v_dist_reg = conv_state.get("vol_district") or conv_state.get("vol_loc") or (loc_name if loc_name != "Shared Location" else "Colombo")
            ...
            vol_veh = (vol_rec.get("transport_mode") if vol_rec else None) or "Motorbike"
            vol_dist = (vol_rec.get("service_area") if vol_rec else None) or (conv_state.get("vol_district") if conv_state else "Colombo")
            clean_vol_dist = routing.resolve_district(vol_dist) or (vol_dist if vol_dist else "Colombo")

            # Check if volunteer is currently executing an active assigned task
            v_tasks = database.get_pickup_tasks_for_volunteer(vol_id) if vol_id else []
            active_ongoing = [t for t in v_tasks if t.get("status") in ["ASSIGNED", "EN_ROUTE", "COLLECTED", "IN_TRANSIT"]]
            if active_ongoing:
                curr_active = active_ongoing[-1]
                import routing_service

                route_res = await routing_service.calculate_task_dynamic_route(curr_active["id"], volunteer_location_override=(lat, lng))
                if route_res.get("success"):
                    dist_km = route_res.get("distance_km", 0.0)
                    dur_min = route_res.get("duration_minutes", 1)
                    cost_lkr = route_res.get("estimated_cost", 0)
                    nav_url = route_res.get("directions_url") or map_link
                    phase_label = "Recipient Delivery" if route_res.get("phase") == "DELIVERY" else "Donor Pickup"
                    dest_name = route_res.get("destination", {}).get("name", "Destination")
                    database.update_pickup_task_logistics(task_id=curr_active["id"], total_distance_km=dist_km, estimated_transport_cost=cost_lkr)
                    reply_text = (
                        f"📍 *Location & Live Route Updated!* 🚚\n\n"
                        f"• 🆔 **Task**: `#{curr_active['id']}`\n"
                        f"• 🎯 **Heading To**: {dest_name} ({phase_label})\n"
                        f"• 📏 **Remaining Road Distance**: {dist_km} km\n"
                        f"• ⏱️ **ETA**: ~{dur_min} mins\n"
                        f"• 💰 **Transport Support**: LKR {cost_lkr}\n\n"
                        f"👉 🗺️ *Turn-by-Turn Navigation:* {nav_url}"
                    )
                    try:
                        database.record_message(phone=from_number, sender="agent", text=reply_text)
                    except Exception:
                    ...
                    send_res = await send_whatsapp_message(to_number=from_number, text=reply_text, reply_to_message_id=message_id)
                    return {
                        "status": "location_saved_active_route_updated",
                        "reply": reply_text,
                        "task_id": curr_active["id"],
            ...
            if not tasks_to_offer:
                ...
                district_dons = [
                    d for d in active_dons if routing.resolve_district(d.get("pickup_location") or d.get("location") or "") == clean_vol_dist
                ]
                all_orgs = database.get_all_organizations()
            ...
            if not org_rec:
                ...
                o_dist_reg = conv_state.get("district") or conv_state.get("city") or (loc_name if loc_name != "Shared Location" else "Colombo")
            ...
            org_dist = (org_rec.get("service_area") or org_rec.get("location") or "") if org_rec else ""
            org_clean_dist = routing.resolve_district(org_dist) or (org_dist if org_dist else "Colombo")
